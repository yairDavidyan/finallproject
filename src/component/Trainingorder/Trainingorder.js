...
function Trainingorder(props) {
    ...
    useEffect(async () => {
        ...
        //סוג אימון
        //אימון קבוצתי
        let trainingorder = await axios.get("http://localhost:3030/Trainingorder/getTraining")
        trainingorder = trainingorder.data

        let typeoftraining = await axios.get("http://localhost:3030/Typeoftraining/getTrainingo")
        typeoftraining = typeoftraining.data

        let groupes = await axios.get("http://localhost:3030/groupes/getAllgroupes")
        groupes = groupes.data

        let grouptraining = await axios.get("http://localhost:3030/Grouptraining/getGrouptraining")
        ...
        await trainingorder.forEach(order => {
            typeoftraining.forEach(type => {
                if (order.nameimon == type.id) {
                    ...
                    arr.push({ id: order.codegroupes, codeGroupe: groupes.filter(p => p.id == order.codegroupes)[0].name, typeTraning: type.sogimon, day: type.nameday, start: 0, finish: 0 })
    ...
    let days = [{ day: "יום ראשון" }, { day: "יום שני" }, { day: "יום שלישי" }, { day: "יום רביעי" }, { day: "יום חמישי" }]
                    ...
                    <div className='l'>
                        ...
                        <div className='col-12'>
                            <div className='List'>
                                {/* className="card" */}
                                {days.map((item) =>
                                    <Card style={{ width: '18rem', height: "35vh", overflow: 'scroll', direction: 'ltr' }} >
                                        ...
                                        <ListGroup className='b'>
                                            ...
                                            {combine && combine.length && combine.map((obj) =>
                                               obj.day == item.day ?
                                                    obj.typeTraning == "אימון יחידני" ?
                                                        <ListGroup.Item className={user?obj.codeGroupe==user.groupeId?'groupColor':'':''}><button onClick={() => bbbb(obj)} className="c">{obj.typeTraning}</button><b>{":"}{obj.codeGroupe}</b></ListGroup.Item>
                                                        :
                                                        obj.typeTraning == "אימון קבוצתי" ?
                                                            <ListGroup.Item className={user?obj.codeGroupe==user.groupeId?'groupColor':'':''}><b>{obj.codeGroupe}</b>{":"}{obj.typeTraning}{":"}{obj.start}{"-"}{obj.finish}</ListGroup.Item>
                                                            :
                                                            <ListGroup.Item className={user?obj.codeGroupe==user.groupeId?'groupColor':'':''}><b>{obj.codeGroupe}</b>{":"}{obj.typeTraning}</ListGroup.Item>
                                                    : ""
                                            )}
